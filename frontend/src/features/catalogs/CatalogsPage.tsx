...
export function CatalogsPage() {
  ...
  const [domainFilter, setDomainFilter] = useState<"Todos" | CatalogDomain>("Todos");
  ...
  const catalogosQuery = useQuery({
    queryKey: ["catalogos", "master-data"],
    queryFn: async () => {
  ...
  const allEntries = catalogosQuery.data ?? [];
  const entries = useMemo(
    () =>
      allEntries.filter(
        (entry) => (entry.dominio?.trim().toUpperCase() ?? "GENERAL") !== "ACTIVOS",
      ),
    [allEntries],
  );
  const domainOptions = useMemo(() => getCatalogDomains(entries), [entries]);
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((entry) => {
      const matchesSearch =
        ...
        normalize(
          [
            formatCatalogDomainLabel(entry.dominio),
            formatCatalogTypeLabel(entry.tipo),
            entry.codigo,
            entry.nombre,
            entry.descripcion,
          ].join(" "),
      ...
      const matchesDomain =
        domainFilter === "Todos" ||
        (entry.dominio?.trim().toUpperCase() ?? "GENERAL") === domainFilter;
  ...
  }, [domainFilter, entries, search, statusFilter, typeFilter]);
    ...
    <section className="catalogs-page">
      <header className="page-header page-header-inline">
        <div>
          ...
          <p className="page-copy">
            Administre los catalogos generales y de tratamientos que alimentan formularios,
            validaciones y reglas del sistema, con codigos unicos y baja logica controlada.
          </p>
          <p className="permission-hint">
            Rol actual: <strong>{roleCapabilities.label}</strong>. Solo administracion puede crear,
            editar o deshabilitar items de catalogo.
          </p>
        </div>
          ...
          {roleCapabilities.catalogs.create ? (
            <button
              ...
              onClick={() => {
                ...
                setDraftCatalog(emptyEntry);
                setActiveCatalogId(emptyEntry.id);
              }}
            >
              Nuevo item de catalogo
            </button>
          ) : null}
        </div>
      </header>
      ...
      <div className="org-toolbar panel">
        <label className="field">
          ...
          <input
            className="input"
            placeholder="Ej. BASE_DATOS, salud, obligacion legal"
            value={search}
        ...
        <label className="field">
          ...
          <select
            className="input"
            value={domainFilter}
            onChange={(event) => setDomainFilter(event.target.value as "Todos" | CatalogDomain)}
          >
            <option value="Todos">Todos</option>
            {domainOptions.map((item) => (
              <option key={item} value={item}>
                {formatCatalogDomainLabel(item)}
              </option>
            ))}
          </select>
        </label>
      ...
      {catalogosQuery.isError ? (
        <section className="panel access-panel">
          ...
          <p className="page-copy">
            No fue posible consultar los catalogos desde el backend. Revise la API antes de
            administrar los datos de referencia del sistema.
          </p>
        </section>
      ) : null}
      ...
      <section className="panel org-admin-single">
        <div className="panel-heading panel-heading-compact">
          <div>
            <span className="brand-kicker">Catalogos</span>
            <h3>Catalogos generales y de tratamientos</h3>
          </div>
            ...
            <tbody>
              {filteredEntries.map((entry) => {
                ...
                return (
                  ...
                  >
                    <td>{formatCatalogDomainLabel(entry.dominio)}</td>
...
function CatalogManagementModal({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="catalog-management-title"
    >
      <button
        type="button"
        className="report-preview-modal-backdrop"
        aria-label="Cerrar gestion de catalogo"
        onClick={onClose}
      />
      ...
      <div className="report-preview-modal-dialog catalog-modal">
        <header className="report-preview-modal-header">
          <div>
            <span className="brand-kicker">Gestion de catalogo</span>
            <div className="page-title-with-icon page-title-with-icon-modal">
              <span className="page-title-icon">
                <AppIcon name="catalogs" size={20} strokeWidth={2.1} />
              </span>
              <h3 id="catalog-management-title">
                {catalog.nombre || "Nuevo item de catalogo"}
              </h3>
            </div>
          </div>
        ...
        <div className="report-preview-modal-body">
          <div className="catalog-modal-grid">
            <div className="detail-block">
              <h4>Edicion del item de catalogo</h4>
              <div className="catalog-form-grid">
                ...
                <label className="field">
                  ...
                  <select
                    className="input"
                    value={catalog.dominio ?? "GENERAL"}
                    onChange={(event) =>
                      onChange({
                        ...catalog,
                        dominio: event.target.value.toUpperCase(),
                      })
                    }
                    disabled={!canUpdate}
                  >
